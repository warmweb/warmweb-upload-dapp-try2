...
export interface ParsedContent {
  ...
  description: string;
  theme: {
    palette: 'blue' | 'emerald' | 'purple' | 'orange';
    font: 'inter' | 'roboto' | 'poppins' | 'montserrat';
  };
  sections: Array<{
    type: 'hero' | 'features' | 'testimonials' | 'pricing' | 'faq' | 'cta' | 'contact' | 'footer';
    content: any;
    animation: 'fade' | 'slide' | 'parallax' | 'zoom' | 'none';
  }>;
  features: {
...
export class AILandingPageGenerator {
  ...
  parsePrompt(prompt: string): ParsedContent {
    ...
    const themeMapping: { [key: string]: ParsedContent['theme'] } = {
    ...
    for (const [sectionType, keywords] of Object.entries(sectionKeywords)) {
      if (keywords.some(keyword => normalizedPrompt.includes(keyword))) {
        sections.push({
          ...
          content: contentTemplates[sectionType as keyof typeof contentTemplates],
          animation: 'fade'
        });
      }
    }
  ...
  private generateTitle(businessType: string): string {
    const titles = {
    ...
    return titles[businessType as keyof typeof titles] || 'Your Business Solution';
  }

  private generateDescription(title: string, businessType: string): string {
    const descriptions = {
    ...
    return descriptions[businessType as keyof typeof descriptions] ||
           `${title} - Professional solutions tailored to your needs.`;
  }

  private getContentTemplates(businessType: string, originalPrompt: string) {
  ...
  previewGeneration(prompt: string): ParsedContent {
